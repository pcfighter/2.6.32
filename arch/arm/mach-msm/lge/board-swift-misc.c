...
extern int aat2870bl_ldo_set_level(struct device * dev, unsigned num, unsigned vol); // what is
extern int aat2870bl_ldo_enable(struct device * dev, unsigned num, unsigned enable); // it?
...
void __init lge_add_misc_devices(void)
{
	...
	//if(lge_bd_rev >= LGE_REV_E) platform_device_register(&msm_device_pmic_leds);
}
